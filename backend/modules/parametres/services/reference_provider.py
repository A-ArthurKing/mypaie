...
def get_all_references():
    ...
    try:
        ...
        with conn.cursor(pymysql.cursors.DictCursor) as cur:
            ...
            # Groupement des KPIs par univers - NOUVELLE METHODE DYNAMIQUE
            # Les KPIs proviennent de config_kpis (la nouvelle table UI)
            cur.execute("""
                SELECT 
                    id, code_kpi as code, libelle, univers, type, formule, is_active as actif
                FROM config_kpis
                ORDER BY libelle
            """)
            ...
            kpis_grouped = {}
            for k in kpis_raw:
                # Fallback : on force tech_key = code_kpi car l'UI pourrait l'utiliser
                k['tech_key'] = k['code']
                u = k['univers']
