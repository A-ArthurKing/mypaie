...
def _run_universal_etl():
    """Wrapper pour lancer Silver puis Gold en arrière-plan."""
    try:
        run_etl_silver()
        run_etl_gold()
        logger.info("ETL Performance (Universal) terminé avec succès.")
    except Exception as e:
        logger.error("Erreur durant l'exécution de l'ETL Universal : %s", e)
...
def endpoint_trigger_etl():
    ...
    try:
        thread = threading.Thread(target=_run_universal_etl)
        thread.start()
