...
def endpoint_update_grilles_order(regle_id):
    try:
        from modules.regles_primes.services.dw_api_regles_provider import update_grilles_order
        data = request.json
...
@regles_primes_bp.route("/api/regles/<int:regle_id>/calcul", methods=["GET"])
def endpoint_calcul_regle(regle_id):
    """
    Déclenche le moteur de calcul unifié pour une règle.
    ...
    if not date_debut or not date_fin or not matricules:
        return jsonify({"error": "Paramètres requis: date_debut, date_fin, matricules"}), 400

    try:
        from modules.regles_primes.services.calculation_engine import run_payout_calculation
        regle = get_regle_by_id(regle_id)
        if not regle:
            return jsonify({"error": "Règle non trouvée"}), 404
            
        results = run_payout_calculation(regle, matricules, date_debut, date_fin)
        return jsonify({"data": results}), 200
    except Exception as e:
        logger.error("Erreur calcul règle %s : %s", regle_id, e)
        return jsonify({"error": str(e)}), 500
